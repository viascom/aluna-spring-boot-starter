...
internal open class InteractionServerSpecificInitializer(
    ...
    private val serverSpecificInteractionManager: ServerSpecificInteractionManager,
    ...
    override fun onApplicationEvent(event: DiscordNodeReadyEvent) {
        AlunaDispatchers.InternalScope.launch {
            if (discordBot.interactionsServerSpecificInitialized) {
                return@launch
            }

            if (alunaProperties.command.serverSpecific.enabled) {
                discordBot.interactionsServerSpecificInitialized = true
                logger.debug("Initialize server specific interactions")
                initServerSpecificCommands()
            }
        }
    }

    suspend fun initServerSpecificCommands() = withContext(AlunaDispatchers.Internal) {
        ...
        if ((filteredCommands.isNotEmpty() || filteredContext.isNotEmpty()) && !alunaProperties.command.serverSpecific.enabled) {
            logger.error("You have configured server specific interactions, but you have not enabled them in your configuration. Please enable them in your configuration as they will otherwise be ignored.")
            return@withContext
        }
            ...
            server.updateCommands().addCommands(relevantCommands).queue { updatedCommands ->
                ...
                updatedCommands.filter { it.type == Type.SLASH }.filter { it.name in filteredCommands.map { it.name } }.forEach { interaction ->
                    try {
                        ...
                        if (filteredCommands.first { it.name == interaction.name }.observeAutoComplete && interaction.id !in discordBot.commandsWithAutocomplete) {
                            discordBot.commandsWithAutocomplete.add(interaction.id)
                        }
                        if (filteredCommands.first { it.name == interaction.name }.handlePersistentInteractions && interaction.id !in discordBot.commandsWithPersistentInteractions) {
                ...
                updatedCommands.forEach { interaction ->
                    if (interaction.timeModified.isAfter(beforeUpdate)) {
                        if (interaction.type == Type.SLASH) {
                            interactionInitializer.printCommand(filteredCommands.first { it.name == interaction.name }, true, arrayListOf(serverId))
                        } else {
        ...
        if (alunaProperties.command.serverSpecific.removeOutdatedInteractionsOnStartup) {
            logger.debug("Removing outdated server-specific interactions. This may take a while...")
            serverSpecificInteractionManager.removeAllOutdatedServerSpecificCommands()
        } else {
            logger.debug("Not removing outdated server-specific interactions as it is disabled in the configuration.")
        }
        ...
        if (alunaProperties.command.systemCommand.enabled && alunaProperties.command.systemCommand.servers != null) {
            alunaProperties.command.systemCommand.servers!!.forEach { serverId ->
                launch {
                    ...
                    val systemCommand = filteredCommands.firstOrNull { it.name == "system-command" } ?: throw IllegalArgumentException()
                    ...
                    server!!.updateCommands().addCommands(systemCommand).coQueue { discordCommands ->
                        ...
                        interactionInitializer.printCommand(systemCommand, true, arrayListOf(serverId))
    ...
    internal fun getFilteredContext(): ArrayList<DiscordContextMenuHandler> {
        val filteredContext = contextMenus
            ...
            .filter { it.specificServers?.isNotEmpty() == true }
            .map {
                try {
