...
suspend fun <T> RestAction<T>.coQueue(success: (suspend CoroutineScope.(T) -> Unit)?, failure: (suspend CoroutineScope.(Throwable) -> Unit)?) = this@coQueue.queue({
    if (success != null) {
...
suspend fun <T> RestAction<T>.coQueueAfter(
    delay: Long,
    unit: TimeUnit,
