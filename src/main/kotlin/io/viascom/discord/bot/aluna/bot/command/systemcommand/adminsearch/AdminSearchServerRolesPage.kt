...
class AdminSearchServerRolesPage(
    ...
    override fun onServerRequest(discordServer: Guild, embedBuilder: EmbedBuilder) {
        ...
        discordServer.roles.sortedByDescending { it.positionRaw }.forEach {
            val newElement =
                (if (it.permissions.any {
                        it in listOf(
                            Permission.ADMINISTRATOR,
                            Permission.MANAGE_SERVER
                        )
                    }) "**" else "") + "- ${it.name}  `${it.id}`" + (if (it.permissions.contains(Permission.ADMINISTRATOR)) " ¹**" else "") + (if (it.permissions.contains(
                        Permission.MANAGE_SERVER
                    )
                ) " ²**" else "")
            if (text.length + newElement.length >= 1000) {
                embedBuilder.addField(if (isFirst) "Roles (${discordServer.roles.size})" else "", text, false)
                text = ""
                isFirst = false
            }
            text += "\n" + newElement
        ...
        embedBuilder.setFooter("¹: Admin | ²: Manage Server")
